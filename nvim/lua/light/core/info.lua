...
local text = require "light.interface.text"
...
local function make_formatters_info(ft)
  local null_formatters = require "light.lsp.null-ls.formatters"
  ...
  local section = {
    ...
    fmt(
      "* Active: %s%s",
      table.concat(registered_formatters, " " .. light.icons.ui.BoxChecked .. " , "),
      vim.tbl_count(registered_formatters) > 0 and " " .. light.icons.ui.BoxChecked .. " " or ""
    ),
...
local function make_code_actions_info(ft)
  local null_actions = require "light.lsp.null-ls.code_actions"
  local registered_actions = null_actions.list_registered(ft)
  local section = {
    "Code actions info",
    fmt(
      "* Active: %s%s",
      table.concat(registered_actions, " " .. light.icons.ui.BoxChecked .. " , "),
      vim.tbl_count(registered_actions) > 0 and " " .. light.icons.ui.BoxChecked .. " " or ""
    ),
  }
...
local function make_linters_info(ft)
  local null_linters = require "light.lsp.null-ls.linters"
  ...
  local section = {
    ...
    fmt(
      "* Active: %s%s",
      table.concat(registered_linters, " " .. light.icons.ui.BoxChecked .. " , "),
      vim.tbl_count(registered_linters) > 0 and " " .. light.icons.ui.BoxChecked .. " " or ""
    ),
...
function M.toggle_popup(ft)
  ...
  local function set_syntax_hl()
    vim.cmd [[highlight lightInfoIdentifier gui=bold]]
    vim.cmd [[highlight link lightInfoHeader Type]]
    vim.fn.matchadd("lightInfoHeader", "Buffer info")
    vim.fn.matchadd("lightInfoHeader", "Active client(s)")
    vim.fn.matchadd("lightInfoHeader", fmt("Overridden %s server(s)", ft))
    vim.fn.matchadd("lightInfoHeader", "Formatters info")
    vim.fn.matchadd("lightInfoHeader", "Linters info")
    vim.fn.matchadd("lightInfoHeader", "Code actions info")
    vim.fn.matchadd("lightInfoHeader", "Automatic LSP info")
    vim.fn.matchadd("lightInfoIdentifier", " " .. ft .. "$")
    vim.fn.matchadd("string", "true")
    vim.fn.matchadd("string", "active")
    vim.fn.matchadd("string", light.icons.ui.BoxChecked)
    vim.fn.matchadd("boolean", "inactive")
    vim.fn.matchadd("error", "false")
    tbl_set_highlight(require("light.lsp.null-ls.formatters").list_registered(ft), "lightInfoIdentifier")
    tbl_set_highlight(require("light.lsp.null-ls.linters").list_registered(ft), "lightInfoIdentifier")
    tbl_set_highlight(require("light.lsp.null-ls.code_actions").list_registered(ft), "lightInfoIdentifier")
  end

  local Popup = require("light.interface.popup"):new {
