...
function plugin_loader.init(opts)
  ...
  if not utils.is_directory(lazy_install_dir) then
    ...
    local core_plugins_dir = join_paths(get_light_base_dir(), "plugins")
    if utils.is_directory(core_plugins_dir) then
      vim.fn.mkdir(plugins_dir, "p")
      vim.fn.delete(plugins_dir, "rf")
      require("light.utils").fs_copy(core_plugins_dir, plugins_dir)
    else
      vim.fn.system {
        "git",
        "clone",
    ...
    vim.api.nvim_create_autocmd("User", { pattern = "LazyDone", callback = require("light.lsp").setup })
  end

  local rtp = vim.opt.rtp:get()
  local base_dir = (vim.env.LUNARVIM_BASE_DIR or get_runtime_dir() .. "/light"):gsub("\\", "/")
